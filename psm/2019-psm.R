################################################################################
# 2019 Propensity Score Matching (PSM) Script
################################################################################
#
# Purpose:
#   This script implements propensity score matching for the 2019 Travel Behavior
...
#
# Steps:
#   1. Load and prepare the 2019 person-day data
...
#
# Input:
#   - data/2019-clean/trip_count_clean_2019.csv
#
# Output:
...
# ── Step 1: Load Data ─────────────────────────────────────────────────────────
# The cleaned 2019 person-day dataset already has all the variables we need.

data_2019 <- read.csv("YOUR FILE PATH")
...
# Harmnize other categorical variables (e.g., education, job_type, work_mode) using the same as in the main model. For example:
...
cat("\nTreatment distribution (telework_binary):\n")
print(table(data_2019$telework_binary))

cat("\nTreatment proportion:", round(mean(data_2019$telework_binary), 3), "\n")
...
love.plot(
  ...
  title        = "Covariate Balance Before and After Matching (2019 Data)",
...
ggplot(psm_data, aes(x = propensity_score,
  ...
  labs(
    title   = "Propensity Score Distribution by Treatment Group (2019)",
