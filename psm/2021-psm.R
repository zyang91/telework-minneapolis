################################################################################
# 2021 Propensity Score Matching (PSM) Script
################################################################################
#
# Purpose:
#   This script implements propensity score matching for the 2021 Travel Behavior
...
#
# Steps:
#   1. Load and prepare the 2021 person-day data
...
#
# Input:
#   - data/2021-clean/trip_count_clean_2021.csv
#
# Output:
...
# ── Step 1: Load Data ─────────────────────────────────────────────────────────
# The cleaned 2021 person-day dataset already has all the variables we need.

data_2021 <- read.csv("YOUR FILE PATH")
...
# Harmonize other variables (e.g., education, job_type, work_mode) using the same recoding logic as the main model.
...
print(table(data_2021$telework_binary))

cat("\nTreatment proportion:", round(mean(data_2021$telework_binary), 3), "\n")
...
love.plot(
  ...
  title        = "Covariate Balance Before and After Matching (2021 Data)",
...
ggplot(psm_data, aes(x = propensity_score,
  ...
  labs(
    title   = "Propensity Score Distribution by Treatment Group (2021)",
...
# export matched_data
# write.csv(matched_data, "data/2021-clean/psm_matched_data_2021.csv", row.names = FALSE)
